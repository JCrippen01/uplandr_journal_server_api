...
from rest_framework import status
from uplandrapi.models import JournalEntry, Dog, Gear, DogEntry
...
class JournalEntryView(ViewSet):
    def list(self, request):
        ...
        user_entry = self.request.query_params.get('entrysbyuser', None)
        subscribed_entrys = self.request.query_params.get('subscribed', None)

        if user_entry is not None:
            entrys = JournalEntry.objects.filter(user=user)

        if subscribed_entrys is not None:
            entrys = JournalEntry.objects.filter(user__author__follower=user)
    ...
    def update(self, request, pk):
        ...
        try:
            ...
            entry.duration = entry.entry.duration
